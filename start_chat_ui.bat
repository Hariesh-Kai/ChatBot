...
IF "%CELERY_DEFAULT_QUEUE%"=="" SET "CELERY_DEFAULT_QUEUE=chatui.default"
...
echo [3/5] Launching Celery Worker (and clearing old tasks)...
start "Chat UI Celery Worker" cmd /k "cd /d %PROJECT_ROOT% && %VENV_NAME%\Scripts\activate && celery -A backend.queue.celery_app:celery_app purge -f && celery -A backend.queue.celery_app:celery_app worker --loglevel=info -Q %CELERY_DEFAULT_QUEUE% --pool=solo --concurrency=1"
