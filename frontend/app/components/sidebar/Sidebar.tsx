...
export default function Sidebar({
  ...
  const filteredChats = useMemo(() => {
    ...
    return visible.filter((c) => (c.title || "").toLowerCase().includes(q) || c.messages.some((m) => (m.content || "").toLowerCase().includes(q)));
  }, [query, chats]);
  ...
  return (
    <>
      {isOpen && <div onClick={() => !isTyping && onClose()} className="fixed inset-0 z-30 bg-black/60 md:hidden" />}
        ...
        {!isOpen && (
          <div className="flex h-full flex-col items-center">
            ...
            <div className="mt-4 flex flex-col gap-3">
              ...
              <PdfUploadButton 
                sessionId={sessionId} iconOnly disabled={uploadDisabled}
        ...
        {isOpen && (
          <div className="flex h-full flex-col">
            ...
            <div className="px-4 py-4 space-y-3">
              ...
              <PdfUploadButton 
                sessionId={sessionId} disabled={uploadDisabled}
                onUploadStart={onUploadStart}
                onUploadSuccess={onUploadSuccess}
                onUploadError={onUploadError}
                onUploadProgress={onUploadProgress} // ✅ Pass it down
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-4">
              <ChatList chats={filteredChats} activeId={activeId} disabled={isTyping} onSelect={(id) => !isTyping && onSelect(id)} onRename={(id) => { const title = prompt("Rename chat"); if (title?.trim()) onRename(id, title.trim()); }} onDelete={onDelete} onPin={onPin} />
            </div>
